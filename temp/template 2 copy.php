...
?>  
  ...
  <li class="item" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
      <div class="item_body">
        <p class="vendor__code">Артикул: <strong id="vendor__code"><?php echo $arItem["PROPERTIES"]["CML2_ARTICLE"]["VALUE"]; ?></strong>
        </p>
    <?if($arItem["PREVIEW_PICTURE"]){?>
    <div class="picture">
      <a href="<?=$arItem["DETAIL_PAGE_URL"]?>"><img class="lazy" data-src="<?=$arItem["PREVIEW_PICTURE"]["SRC"]?>" src="<?=$arItem["PREVIEW_PICTURE"]["SRC"]?>" alt="<?=$arItem["PREVIEW_PICTURE"]["ALT"]?>"></a>
    </div>
    <?}?>
      <div class="item_text">
    <div class="text">
    ...
    <div class="extra">
      <div class="data">
        <?if($arItem["MIN_PRICE"]["VALUE"]){?>
            <div class="price-block">
              ...
              <div class="old-price regular__price"><?php echo $price_normal; ?><span class="symbol"> &#8381;</span></div>
              <?php } ?>
              <div class="price"><?php echo $price_itogo;?><span class="symbol"> &#8381;</span></div>
            </div>
        <?}?>
      ...
      <?if($arItem["MIN_PRICE"]["VALUE"]){?>
      <a href="#" class="buy-btn add_to_basket" data-id="<?=$arItem["ID"]?>"><span><?=GetMessage($l_prefix ."BUY_BTN")?></span></a>
      <?}?>
      <!--
      ...
      <?}?>-->
         <?php if( $arItem["PROPERTIES"]["NEWPRODUCT"]["VALUE"] == "да" ){ ?>
        	<ul class="tags">
      		<li class="tag" style="background: #28a1dc ; border-color: transparent #528dca"><?php echo $arResult["PROPERTIES"]["NEWPRODUCT"]["NAME"]; ?></li>
        	</ul>
        <?php } ?>
    </div>
        </div> 
